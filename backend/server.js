...
app.get("/api/crypto/salt", verifyToken, async (req, res) => {
  try {
    ...
    // Node.js crypto 환경을 위한 난수 생성 가능하나, 편의상 빠른 구현 적용
...
app.delete("/api/admin/records/:id", verifyToken, checkAccess("member_db"), async (req, res) => {
   try {
     ...
     await logAction(req.user.uid, req.user.email, req.user.name, "HARD_DELETE", `RECORD:${req.params.id}`, "요청에 의한 영구 파기");
     res.json({ success: true });
   } catch(err) {
      res.status(500).json({ error: "파기 실패" });
   }
});
