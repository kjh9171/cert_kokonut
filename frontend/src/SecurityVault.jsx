...
import {
  ...
  ChevronDown, Info, Zap, Eye, EyeOff, FileLock, Check, Square, CheckSquare
} from 'lucide-react';
...
const CIPHER_OPTIONS = [
  { id: 'aes256',  label: 'AES-256-GCM', desc: 'NIST 권장 군사급 대칭키 암호화.', level: 'MILITARY', color: 'emerald' },
  { id: 'aes128',  label: 'AES-128-GCM', desc: '경량 대칭키 암호화. 빠른 속도.', level: 'STRONG',   color: 'blue' },
  { id: 'custom',  label: '패스프레이즈 기반', desc: 'PBKDF2(SHA-256) 키 파생.', level: 'CUSTOM',   color: 'indigo' },
  { id: 'mask',    label: '마스킹 (비가역)', desc: '*** 형식 치환. 복원 불가.', level: 'BASIC',    color: 'amber' },
];

// --- Web Crypto API 암호화 유틸리티 ---
const buf2hex = (buf) => Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2,'0')).join('');

async function deriveKey(pass, salt, bits=256) {
  const enc = new TextEncoder();
  const km = await crypto.subtle.importKey('raw', enc.encode(pass), 'PBKDF2', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name:'PBKDF2', salt: enc.encode(salt), iterations:100000, hash:'SHA-256' },
    km, { name:'AES-GCM', length: bits }, false, ['encrypt']
  );
}

async function genKey(bits=256) {
  return crypto.subtle.generateKey({ name:'AES-GCM', length: bits }, true, ['encrypt']);
}

async function encryptValue(val, key) {
...
// onProcessComplete: 암호화 완료 시 부모(App)에 결과 데이터를 전달하는 콜백
export default function SecurityVault({ onProcessComplete }) {
  ...
  // 3. 데이터 + 선택 상태
  const [allData, setAllData] = useState([]); // [header, ...rows]
  ...
  const handleProcess = async () => {
    if (selectedCipher === 'custom' && !passphrase.trim()) { alert('패스프레이즈를 입력해 주세요.'); return; }
    if (selectedCols.size === 0 || selectedRows.size === 0) { alert('암호화할 필드와 레코드를 선택해 주세요.'); return; }
    ...
    try {
      ...
      if (selectedCipher === 'aes256') { cryptoKey = await genKey(256); exportedHex = buf2hex(await crypto.subtle.exportKey('raw', cryptoKey)); }
      else if (selectedCipher === 'aes128') { cryptoKey = await genKey(128); exportedHex = buf2hex(await crypto.subtle.exportKey('raw', cryptoKey)); }
      else if (selectedCipher === 'custom') { const salt = `pms-${Date.now()}`; cryptoKey = await deriveKey(passphrase, salt); exportedHex = `PASSPHRASE (salt:${salt})`; }
        ...
        for (let ci = 0; ci < srcRow.length; ci++) {
          if (selectedRows.has(ri) && selectedCols.has(ci)) {
            ...
            if (selectedCipher === 'mask') {
              newRow[ci] = maskValue(srcRow[ci]);
            } else {
              newRow[ci] = await encryptValue(srcRow[ci], cryptoKey);
            }
            done++;
            setProgress(Math.floor((done / totalTarget) * 100));
            // 셀별 처리 시각화를 위해 미세 딜레이
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(processed), 'Secured');
      ...
      if (exportedHex) {
        ...
        setKeyBlobUrl(URL.createObjectURL(new Blob([kt], { type: 'text/plain;charset=utf-8' })));
      }

      setStats({ rows: selectedRows.size, cells: done });
      setPhase('done');

      // 부모(App) 컴포넌트에 결과 전달 -> 회원DB 탭 연동
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-5 duration-500 pb-20">
      ...
      <div className="flex justify-between items-end">
        ...
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2"><span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span> Web Crypto API</span>
      </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm">
              <h3 className="font-black text-slate-800 mb-5 uppercase italic text-xs tracking-widest flex items-center gap-2"><Key size={14} className="text-blue-500" /> Cipher Protocol</h3>
              <div className="space-y-2">
                {CIPHER_OPTIONS.map(opt => (
                  <button key={opt.id} onClick={() => setSelectedCipher(opt.id)}
                    className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${selectedCipher === opt.id ? `bg-${opt.color}-50/50 border-${opt.color}-200` : 'bg-slate-50 border-transparent hover:border-slate-200'}`}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border uppercase tracking-widest bg-${opt.color}-50 text-${opt.color}-600 border-${opt.color}-100`}>{opt.level}</span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {headers.map((h, ci) => (
                      <button key={ci} onClick={() => phase === 'selecting' && toggleCol(ci)}
                        ...
                        {selectedCols.has(ci) ? <Check size={12} className="inline mr-1" /> : null}
                        {h || `Col ${ci+1}`}
                      </button>
                    ))}
                  </div>
                ...
                <div className="overflow-x-auto rounded-2xl border border-slate-100 max-h-[400px] overflow-y-auto">
                      ...
                      {dataRows.slice(0, 50).map((row, ri) => (
                        <tr key={ri} className={`transition-all ${selectedRows.has(ri) ? 'bg-white' : 'bg-slate-50/30 opacity-50'}`}>
                          ...
                          {row.map((cell, ci) => {
                            ...
                            const isProcessing = processingCell?.row === ri && processingCell?.col === ci;
                            return (
                              <td key={ci} className={`py-3 px-4 font-medium whitespace-nowrap max-w-[150px] overflow-hidden text-ellipsis transition-all duration-300
                                ${isProcessing ? 'bg-blue-600 text-white font-black animate-pulse scale-105 rounded-lg shadow-lg' : ''}
                                ${isTarget && !isProcessing && phase === 'selecting' ? 'bg-blue-50/60 text-blue-800' : ''}
                  ...
                  {dataRows.length > 50 && (
                    <div className="px-4 py-2 bg-slate-50 text-[10px] text-slate-400 font-black uppercase tracking-widest border-t border-slate-100 text-center">
                      ... 외 {dataRows.length - 50}행 (총 {dataRows.length}행 모두 처리됩니다)
                    </div>
                  )}
                </div>
            ...
            {phase === 'done' && (
              <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm animate-in slide-in-from-bottom-5 duration-500">
                ...
                <div className="space-y-3 mb-8">
                  <a href={resultBlobUrl} download={`SECURED_${uploadedFile?.name}`}
                    ...
                    <div className="flex items-center gap-3">
                      <Download size={20} />
                      <span className="uppercase italic text-sm">암호화 파일 다운로드</span>
                    </div>
                    <ChevronDown size={18} className="rotate-[-90deg] group-hover:translate-x-1 transition-transform" />
                  </a>
                  {keyBlobUrl && (
                    <a href={keyBlobUrl} download={`KEY_${uploadedFile?.name}.txt`}
