...
import { 
  ...
  Smartphone, EyeOff, Share2, Clock, ExternalLink, Scale, Info, FileSearch, Save, Trash2, RefreshCw, UserCheck, Edit3, Send
} from 'lucide-react';
  ...
  <div className="min-h-screen bg-white animate-in fade-in duration-700">
    ...
    <footer className="mt-40 py-24 bg-slate-50 border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-10 flex flex-col items-center text-center">
        ...
        <div className="flex flex-wrap justify-center gap-10 text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-10"><span>개인정보처리방침</span><span>서비스 이용약관</span><span>쿠키정책</span></div>
        <p className="text-slate-400 text-[10px] font-bold uppercase">주식회사 개인정보관리서비스 | 대표이사: 김종환 | 사업자번호: 000-00-00000</p>
        ...
        return (
          <div className="space-y-8 animate-in slide-in-from-bottom-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[{ l: '총 보관 자산', v: dashStats.total.toLocaleString(), c: 'text-blue-600', i: Database }, { l: '정보 동의율', v: (dashStats.consentRate || 98) + '%', c: 'text-emerald-600', i: ShieldCheck }, { l: '금일 보안 활동', v: String(dashStats.today || 0), c: 'text-indigo-600', i: Activity }, { l: '라이선스 잔여', v: '184일', c: 'text-amber-600', i: CreditCard }].map((s, idx) => (
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
              ...
              <div className="overflow-x-auto text-xs"><table className="w-full text-left border-collapse"><thead><tr className="border-b-2 border-slate-50 text-slate-400 font-black uppercase">
                <th className="py-4 px-4">일시</th><th className="py-4 px-4">담당자</th><th className="py-4 px-4">작업</th><th className="py-4 px-4">대상 자산</th><th className="py-4 px-4">수행 사유</th>
              </tr></thead><tbody className="divide-y divide-slate-50">
                {auditLogs.length === 0 ? <tr><td colSpan={5} className="py-10 text-center text-slate-400">보안 활동 없음</td></tr> : auditLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-4"><span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${log.action === 'ENCRYPT' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>{log.action === 'ENCRYPT' ? '암호화' : '복호화'}</span></td>
      ...
      case 'user_manage':
        return (
          <div className="space-y-8 animate-in zoom-in-95 duration-500">
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm"><div className="overflow-x-auto"><table className="w-full text-left border-collapse text-sm font-bold"><thead><tr className="border-b-2 border-slate-50 text-[10px] text-slate-400 uppercase tracking-widest">
              <th className="py-4 px-6">성명</th><th className="py-4 px-6">이메일</th><th className="py-4 px-6">권한</th><th className="py-4 px-6">등록일시</th><th className="py-4 px-6 text-right">관리</th>
            </tr></thead><tbody className="divide-y divide-slate-50 text-slate-700">
              {users.map(u => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-5 px-6 italic uppercase">{u.name}</td>
                  <td className="py-5 px-6 font-medium text-slate-500">{u.email}</td>
                  <td className="py-5 px-6"><span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${u.role === 'admin' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>{u.role === 'admin' ? '관리자' : '일반'}</span></td>
                  <td className="py-5 px-6 text-xs text-slate-400">{new Date(u.createdAt).toLocaleDateString()}</td>
                  <td className="py-5 px-6 text-right flex justify-end gap-3 text-slate-300"><Trash2 size={18} className="cursor-pointer hover:text-rose-600" onClick={() => {if(confirm('삭제하시겠습니까?')) authFetch(`/api/admin/admins/${u.id}`, {method:'DELETE'}).then(()=>loadUsers());}} /></td>
                </tr>
              ))}
            </tbody></table></div></div>
          </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] flex animate-in fade-in duration-700 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 px-8 flex justify-between items-center sticky top-0 z-30">
          <div className="flex items-center gap-6"><button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2.5 hover:bg-slate-50 rounded-xl text-slate-400 border border-slate-100 shadow-sm"><Menu size={20} /></button><SessionTimer initialTime={4800} onLogout={onLogout} /></div>
          <div className="flex items-center gap-4"><div className="text-right hidden sm:block"><p className="text-sm font-black text-slate-800 uppercase italic">보안 관리자 (대표님)</p><div className="flex justify-end items-center gap-1.5 mt-0.5"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span><p className="text-[9px] text-emerald-500 font-bold uppercase tracking-widest">시스템 인증됨</p></div></div><div className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-200 text-slate-500"><Users size={18} /></div></div>
        </header>
...
export default function App() {
  ...
  // 회원가입 관련 상태
  const [signupStep, setSignupStep] = useState('email'); // 'email' | 'verify' | 'info'
  ...
  useEffect(() => { if (currentScreen === 'company_admin' && authToken) loadData(); }, [currentScreen, authToken, loadData]);
