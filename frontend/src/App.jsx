...
import { 
  ...
  Smartphone, EyeOff, Share2, Clock, ExternalLink, Scale, Info, FileSearch, Save, Trash2, RefreshCw, UserCheck, Edit3, Send, User, ChevronLeft, CreditCard as CardIcon, Calendar, QrCode
} from 'lucide-react';
...
const SessionTimer = ({ initialTime, onLogout, isSandbox }) => {
  ...
  return (
    <div className="flex items-center gap-3 bg-blue-50 px-4 py-2 rounded-full border border-blue-100 shadow-inner whitespace-nowrap">
      ...
      <button onClick={() => setTimeLeft(3600)} className="text-[10px] font-black bg-blue-600 text-white px-2 py-0.5 rounded ml-2 hover:bg-blue-700 transition shadow-sm border border-blue-500 uppercase">Renew</button>
    </div>
  );
};
...
const CompanyAdminView = ({ 
  ...
  const startOtpSetup = async () => {
    ...
    setOtpSetup({ ...otpSetup, open: true, qrCode: data.qrCodeUrl, secret: data.secret });
  };
  ...
  const disableOtp = async () => {
    if (!confirm('2단계 인증을 해제하시겠습니까? 보안이 취약해질 수 있습니다.')) return;
  ...
  const renderContent = () => {
    ...
    if (isExpired && activeTab !== 'my_settings') return <RestrictedView title="라이선스 만료됨" message="보안 서비스 이용 기간이 종료되었습니다. 계속 이용하시려면 라이선스를 갱신하거나 구독을 연장해 주십시오." onUpgrade={() => alert('결제 시스템 준비 중')} />;
    if (!hasPermission && !isAdmin) return <RestrictedView title="접근 권한 제한" message="해당 기능은 현재 대표님의 권한 등급에서는 접근이 제한되어 있습니다. 업그레이드를 통해 모든 보안 기능을 활성화하세요." onUpgrade={() => alert('구독 업그레이드')} />;
              ...
              <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
                ...
                <div className="overflow-x-auto text-xs"><table className="w-full text-left border-collapse font-bold"><thead><tr className="border-b-2 border-slate-50 text-slate-400 font-black uppercase"><th className="py-4 px-4">일시</th><th className="py-4 px-4">담당자</th><th className="py-4 px-4">작업</th><th className="py-4 px-4">대상 자산</th><th className="py-4 px-4">수행 사유</th></tr></thead><tbody className="divide-y divide-slate-50 text-slate-700">
                  {auditLogs.length === 0 ? <tr><td colSpan={5} className="py-10 text-center text-slate-400 italic font-medium">기록된 보안 활동이 없습니다.</td></tr> : auditLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors"><td className="py-4 px-4 text-slate-400 tabular-nums">{new Date(log.createdAt).toLocaleString('ko-KR')}</td><td className="py-4 px-4">{log.userName}</td><td className="py-4 px-4"><span className={`px-3 py-1 rounded-full text-[9px] font-black ${log.action === 'ENCRYPT' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>{log.action === 'ENCRYPT' ? '암호화' : '복호화'}</span></td><td className="py-4 px-4 italic font-medium">{log.target}</td><td className="py-4 px-4 font-medium truncate max-w-xs">{log.reason}</td></tr>
                  ))}
                </tbody></table></div>
              </div>
            )}
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
              <div className="overflow-x-auto text-sm font-bold text-slate-700"><table className="w-full text-left border-collapse"><thead><tr className="border-b-2 border-slate-50 text-[10px] text-slate-400 uppercase tracking-widest"><th className="py-4 px-6">성명</th><th className="py-4 px-6">이메일</th><th className="py-4 px-6">권한/상태</th><th className="py-4 px-6">라이선스 키</th><th className="py-4 px-6">만료일</th><th className="py-4 px-6 text-right">관리</th></tr></thead><tbody className="divide-y divide-slate-50">
                {isSandbox ? <tr><td colSpan={6} className="py-10 text-center italic text-slate-400">체험 모드 이용자 조회 불가</td></tr> : users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-50 transition-colors"><td className="py-5 px-6 italic uppercase">{u.name}</td><td className="py-5 px-6 font-medium text-slate-500">{u.email}</td><td className="py-5 px-6"><span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${u.role === 'admin' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>{u.role === 'admin' ? '관리자' : '일반'}</span></td><td className="py-5 px-6 text-xs font-mono text-slate-400">{u.licenseKey || 'N/A'}</td><td className="py-5 px-6 text-xs text-slate-400">{u.licenseExpiry ? new Date(u.licenseExpiry).toLocaleDateString() : '-'}</td><td className="py-5 px-6 text-right flex justify-end gap-3 text-slate-300"><Edit3 size={18} className="cursor-pointer hover:text-blue-600" onClick={() => setUserModal({ open: true, type: 'edit', data: { ...u, permissions: u.permissions || ['dashboard'], password: '', licenseExpiry: u.licenseExpiry || '' } })} /><Trash2 size={18} className="cursor-pointer hover:text-rose-600" onClick={() => {if(confirm('삭제하시겠습니까?')) authFetch(`/api/admin/admins/${u.id}`, {method:'DELETE'}).then(()=>loadUsers());}} /></td></tr>
                ))}
              </tbody></table></div>
            </div>
            {userModal.open && (
            ...
            {otpSetup.open && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6 animate-in fade-in">
                <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-10 relative text-center">
                  ...
                  <div className="space-y-4"><input type="text" maxLength={6} value={otpSetup.code} onChange={e=>setOtpSetup({...otpSetup, code: e.target.value})} placeholder="인증코드 6자리" className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-black text-center tracking-[0.5em] text-xl" /><button onClick={enableOtp} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-lg hover:bg-blue-700">활성화하기</button></div>
                </div>
              </div>
            )}
          </div>
    ...
    <div className="h-screen bg-[#f8fafc] flex animate-in fade-in duration-700 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 px-8 flex justify-between items-center sticky top-0 z-30 shrink-0">
          <div className="flex items-center gap-6"><button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2.5 hover:bg-slate-50 rounded-xl text-slate-400 border border-slate-100 shadow-sm"><Menu size={20} /></button><SessionTimer initialTime={3600} onLogout={onLogout} isSandbox={isSandbox} /></div>
          ...
          <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 animate-in fade-in zoom-in duration-500">
            <div className="w-full max-w-[440px] bg-white rounded-[4rem] shadow-2xl p-14 border border-slate-100 relative">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                <div className="flex items-center justify-center gap-4 py-2 border-y border-slate-50"><button type="button" onClick={()=>alert('구글 로그인 준비 중')} className="flex items-center gap-2 text-[10px] font-black text-slate-400 hover:text-slate-900 transition"><Globe size={14}/> Google 계정으로 접속</button></div>
